/**
 * Controller per la gestione delle carte (debito e prepagate).
 *
 * Fornisce operazioni:
 * - creazione di carte e generazione OTP
 * - recupero delle carte dell'utente o di tutte le carte
 * - calcolo entrate/uscite di una carta
 * - verifica OTP e reinvio OTP
 * - cancellazione di carte
 * - generazione di numeri di carta e CVV validi
 *
...
abstract class CardController {
  ...
  /**
   * Creazione di una nuova carta (debito o prepagata).
   * - Verifica se il titolare esiste, altrimenti lo crea
   * - Per carte di debito, controlla il conto bancario associato
  ...
  static async create(req:CreateRequest, res:Response) {
    try {
      ...
      await Utilities.followSession(null, async(session) => {
        // Recupera o crea il titolare della carta
        ...
        // Associa il titolare all'utente
        await UserModel.getInstance().updateById(user.id, {
          $addToSet: { holders: holder.id },
        }, {session})

        let newCard!:CardDocument;
        const otp = await Otp.generate(); // Genera OTP per verifica carta

        if(type === CardType.debit) {
          // Controlli per carte di debito
          ...
          // Limite massimo 4 carte per conto
          ...
          // Limite massimo 4 carte prepagate per utente
        ...
        // Risposta senza dati sensibili della carta
  ...
  /**
   * Recupero carte dell'utente.
   * - Se id è presente, restituisce la singola carta
   * - Altrimenti restituisce tutte le carte dell'utente
   * - Filtra informazioni sensibili se in stato pending_verification
   */
  ...
  /**
   * Recupero carta singola o tutte le carte (usato da admin o query globale)
   */
  ...
  /**
   * Calcolo entrate (inc) e uscite (exp) di una carta.
   * - Differenzia carte debit e prepaid
   * - Usa tipo transazione per determinare se sommare a inc o exp
   */
  static async getIncAndExp(req:GetIncExpRequest, res:Response) {
    try {
      ...
      const transactions = await TransactionModel.getInstance().getByCardId([id], { createdAt: { $gte: startOfYear } });
      const incExp = {inc: 0, exp: 0};

      transactions.forEach((element) => {
        if(CardModel.isDebitType(card)) {
          // Carta di debito: basato su conto associato
          ...
          // Carta prepagata: basato su carte source/destination
  ...
  /**
   * Eliminazione di più carte tramite array di ID
   */
  ...
  /**
   * Verifica OTP della carta
   * - Aggiorna stato carta e titolare
   * - Attiva conto se la carta è di debito e il conto era pending
   */
  static async verifyOtp(req:VerifyOtpRequest, res:Response) {
    try {
      ...
      // Salvataggio stato attivo con sessione
      await Utilities.followSession(null, async(session) => {
  ...
  /**
   * Genera un numero di carta valido (algoritmo Luhn)
   */
